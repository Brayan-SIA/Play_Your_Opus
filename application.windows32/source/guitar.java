      ...
      case SELECT_RECORD:            /*レコード曲選択*/
        ...
        if (List_music.size() > 0) {     //曲のリストに一つ以上曲が入っていたら
          ...
          if (pick) {                      //弦が押されたら
            ...
            sampler.rate.setLastValue(0.5f);
...
class Note            /*ノーツのクラス*/
{
  ...
  public void pw_displayPlay()
  {
    ...
    if(time_finish > 0){
      ...
      if(end == true && end_finish != true){  //長押し中だったら
        if(f_1 || f_2 || f_3){
          if(f_1){
            pw.image(image_long_note1, f1_x, pos_finish+(height-note_line-pos_finish)/2, width/5, height-note_line-pos_finish);
          }
          if(f_2){
            pw.image(image_long_note2, f2_x, pos_finish+(height-note_line-pos_finish)/2, width/5, height-note_line-pos_finish);
          }
          if(f_3){
            pw.image(image_long_note3, f3_x, pos_finish+(height-note_line-pos_finish)/2, width/5, height-note_line-pos_finish);
          }
        }
        else{
          pw.image(image_long_note4, f2_x, pos_finish+(height-note_line-pos_finish)/2, width*4/5, height-note_line-pos_finish);
        }
      }
      else{  //流れてるだけの場合
        if(f_1 || f_2 || f_3){
          if(f_1){
            pw.image(image_long_note1, f1_x, pos_finish+(pos-pos_finish)/2, width/5, pos-pos_finish);
          }
          if(f_2){
            pw.image(image_long_note2, f2_x, pos_finish+(pos-pos_finish)/2, width/5, pos-pos_finish);
          }
          if(f_3){
            pw.image(image_long_note3, f3_x, pos_finish+(pos-pos_finish)/2, width/5, pos-pos_finish);
          }
        }
        else{
          pw.image(image_long_note4, f2_x, pos_finish+(pos-pos_finish)/2, width*4/5, pos-pos_finish);
        } 
      }
    }
    /*通常ノーツの表示*/
    ...
    if(is){
      if(f_1 || f_2 || f_3){
        if(f_1){
          pw.image(image_note1, f1_x, pos, width/5, height/100);
        }
        if(f_2){
          pw.image(image_note2, f2_x, pos, width/5, height/100);
        }
        if(f_3){
          pw.image(image_note3, f3_x, pos, width/5, height/100);
        }
      }
      else{
        pw.image(image_note4, f2_x, pos, width*4/5, height/100);
      }
    }
    /*長押しノーツ終了部分の表示*/
    if(is_finish){
      if(f_1 || f_2 || f_3){
        if(f_1){
          pw.image(image_note1, f1_x, pos_finish, width/5, height/100);
        }
        if(f_2){
          pw.image(image_note2, f2_x, pos_finish, width/5, height/100);
        }
        if(f_3){
          pw.image(image_note3, f3_x, pos_finish, width/5, height/100);
        }
      }
      else{
        pw.image(image_note4, f2_x, pos_finish, width*4/5, height/100);
      } 
    }
  }
  ...
  /*評価テキストの表示*/
  public void pw_displayEva()
  {
    int effect_size = (50-eva_count)*width/200;
    /*判定テキストの表示*/
    switch(eva){
      case 0:
        if(eva_count > 25) pw.fill(150,(50-eva_count)*5);  //だんだん濃くする
        else pw.fill(150, eva_count * 10);  //だんだん薄くする
        pw.textSize(width/45);
        if(f_1) pw.text("MISS", f1_x, height-eva_pos);
        if(f_2) pw.text("MISS", f2_x, height-eva_pos);
        if(f_3) pw.text("MISS", f3_x, height-eva_pos);
        if(!f_1 && !f_2 && !f_3) pw.text("MISS", f2_x, height-eva_pos);
        break;
      case 1:
        if(eva_count > 25) pw.fill(color(255,45,150),(50-eva_count)*5);  //だんだん濃くする
        else pw.fill(color(255,45,150), eva_count * 10);  //だんだん薄くする
        pw.textSize(width/35);
        if(f_1){
          pw.text("PERFECT!!", f1_x, height-eva_pos);
          pw.image(image_perfect, f1_x, height - note_line, effect_size, effect_size);
        }
        if(f_2) {
          pw.text("PERFECT!!", f2_x, height-eva_pos);
          pw.image(image_perfect, f2_x, height - note_line, effect_size, effect_size);
        }
        if(f_3){
          pw.text("PERFECT!!", f3_x, height-eva_pos);
          pw.image(image_perfect, f3_x, height - note_line, effect_size, effect_size);
        }
        if(!f_1 && !f_2 && !f_3){
          pw.text("PERFECT!!", f2_x, height-eva_pos);
          pw.image(image_perfect, f2_x, height - note_line, effect_size*2, effect_size*2);
        }
        break;
      case 2:
        if(eva_count > 25) pw.fill(color(255,191,127),(50-eva_count)*5);  //だんだん濃くする
        else pw.fill(color(255,191,127), eva_count * 10);  //だんだん薄くする
        pw.textSize(width/40);
        if(f_1){
          pw.text("GREAT!", f1_x, height-eva_pos);
          pw.image(image_great, f1_x, height - note_line, effect_size, effect_size);
        }
        if(f_2) {
          pw.text("GREAT!", f2_x, height-eva_pos);
          pw.image(image_great, f2_x, height - note_line, effect_size, effect_size);
        }
        if(f_3){
          pw.text("GREAT!", f3_x, height-eva_pos);
          pw.image(image_great, f3_x, height - note_line, effect_size, effect_size);
        }
        if(!f_1 && !f_2 && !f_3){
          pw.text("GREAT!", f2_x, height-eva_pos);
          pw.image(image_great, f2_x, height - note_line, effect_size*2, effect_size*2);
        }
        break;
      case 3:
        if(eva_count > 25) pw.fill(255,(50-eva_count)*5);  //だんだん濃くする
        else pw.fill(255, eva_count * 10);  //だんだん薄くする
        pw.textSize(width/40);
        if(f_1){
          pw.text("GOOD", f1_x, height-eva_pos);
          pw.image(image_good, f1_x, height - note_line, effect_size, effect_size);
        }
        if(f_2) {
          pw.text("GOOD", f2_x, height-eva_pos);
          pw.image(image_good, f2_x, height - note_line, effect_size, effect_size);
        }
        if(f_3){
          pw.text("GOOD", f3_x, height-eva_pos);
          pw.image(image_good, f3_x, height - note_line, effect_size, effect_size);
        }
        if(!f_1 && !f_2 && !f_3){
          pw.text("GOOD", f2_x, height-eva_pos);
          pw.image(image_good, f2_x, height - note_line, effect_size*2, effect_size*2);
        }
        break;   
    }
    eva_pos+=width/400;
...
public class SecondApplet extends PApplet
{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "guitar" };
    if (passedArgs != null) {
